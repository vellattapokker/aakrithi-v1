...
function checkPincode() {
    ...
    setTimeout(() => {
        ...
        // Pseudo logic: Make certain pincodes 'out of service' purely for demo
        if (input.startsWith('99')) {
            result.style.color = '#ef4444';
            message = 'Currently, we do not deliver to this pincode. We are expanding quickly!';
            result.innerHTML = `<i data-lucide="map-pin-off" style="width:18px;height:18px; flex-shrink:0;"></i> <span>${message}</span>`;
        } else {
            message = `Standard Delivery to <strong>${input}</strong> available in 3-5 business days.<br>Cash on Delivery is eligible.`;
            result.innerHTML = `<i data-lucide="check-circle" style="width:18px;height:18px; flex-shrink:0;"></i> <span>${message}</span>`;
        }
